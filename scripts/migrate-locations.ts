...
if (!supabaseUrl || !supabaseKey || supabaseUrl === 'your_supabase_project_url') {
  console.error('Please configure your Supabase credentials in .env.local file');
  process.exit(1);
}
...
async function migrateLocations() {
  try {
    ...
    if (!locationsData || !locationsData.data || !Array.isArray(locationsData.data)) {
      console.error('❌ Invalid locations.json format. Expected { data: Location[] }');
      process.exit(1);
    }
    ...
    const validLocations = locations.filter(location => location.code !== 'CORP');
    console.log(`✅ ${validLocations.length} valid locations (filtered out corporate)`);
    ...
    if (existingLocations && existingLocations.length > 0) {
      console.log('⚠️  Locations table already contains data. Skipping migration.');
      console.log('   To force re-migration, please truncate the locations table first.');
      return;
    }
    ...
    for (let i = 0; i < totalBatches; i++) {
      ...
      console.log(`📤 Inserting batch ${i + 1}/${totalBatches} (${batch.length} locations)...`);
    ...
    console.log('\n📝 Next steps:');
    console.log('   1. Update your .env.local file with your actual Supabase credentials');
