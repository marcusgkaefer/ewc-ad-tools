...
async function addTargetingColumns(): Promise<void> {
  console.log('🔧 Adding missing targeting columns to location_configs table...\n');
  ...
  try {
    ...
    if (migrationError) {
      ...
      const columns = [
        ...
        'ALTER TABLE location_configs ADD COLUMN IF NOT EXISTS landing_page_url TEXT'
         ...
         console.log(`   Adding column: ${columnSQL.split('ADD COLUMN IF NOT EXISTS ')[1]}`);
         // We'll need to use a different approach since Supabase client doesn't allow raw DDL
       }
      
      console.log('❌ Unable to execute DDL statements through Supabase client.');
    ...
    if (infoError) {
      console.log('⚠️  Could not verify table structure, but migration should have succeeded');
    } else if (tableInfo) {
