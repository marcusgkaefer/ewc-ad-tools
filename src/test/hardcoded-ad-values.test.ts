...
describe('Hard-coded Ad Values', () => {
  describe('Reference Ad Template', () => {
    ...
    it('should have valid media hashes and URLs', () => {
      ...
      expect(REFERENCE_AD_TEMPLATE.videoThumbnailUrl).toMatch(/^https:\/\/scontent-/)
      expect(REFERENCE_AD_TEMPLATE.additionalVideoThumbnailUrl).toMatch(/^https:\/\/scontent-/)
    })

    it('should have proper video settings', () => {
      expect(REFERENCE_AD_TEMPLATE.videoId).toBe('v:1794899587789121')
      expect(REFERENCE_AD_TEMPLATE.additionalVideoId).toBe('v:1243573153921320')
      expect(REFERENCE_AD_TEMPLATE.videoPlacement).toContain('facebook biz_disco_feed')
      expect(REFERENCE_AD_TEMPLATE.additionalVideoPlacement).toContain('facebook facebook_reels')
    })

    it('should have proper placement settings', () => {
      const expectedPlacement = 'Default, Default, Default, Default, audience_network classic, facebook biz_disco_feed, facebook facebook_reels, facebook facebook_reels_overlay, facebook feed, facebook instream_video, facebook marketplace, facebook right_hand_column, facebook search, facebook story, facebook video_feeds, instagram explore, instagram reels, instagram story, instagram stream, messenger story'
  ...
  describe('Reference Campaign Settings', () => {
    ...
    it('should have correct budget and bidding settings', () => {
      ...
      expect(REFERENCE_CAMPAIGN_SETTINGS.bidStrategy).toBe('Highest volume or value')
    })

    it('should have proper campaign technical settings', () => {
      expect(REFERENCE_CAMPAIGN_SETTINGS.campaignObjective).toBe('Outcome Engagement')
  ...
  describe('Reference AdSet Settings', () => {
    ...
    it('should have proper targeting settings', () => {
      ...
      expect(REFERENCE_ADSET_SETTINGS.optimizedConversionTrackingPixels).toBe('tp:1035642271793092')
    ...
    it('should have correct demographic targeting', () => {
      ...
      expect(REFERENCE_ADSET_SETTINGS.excludedCustomAudiences).toBe('120213927766160508:AUD-FBAllPriorServicedCustomers')
    })
    ...
    it('should have valid interest targeting JSON', () => {
      ...
      const waxingInterest = interests[0].interests.find((i: { name: string; id: string }) => i.name === 'Waxing')
      expect(waxingInterest).toBeDefined()
      expect(waxingInterest.id).toBe('6002997877444')
    })

    it('should have proper optimization settings', () => {
      expect(REFERENCE_ADSET_SETTINGS.optimizationGoal).toBe('OFFSITE_CONVERSIONS')
      expect(REFERENCE_ADSET_SETTINGS.billingEvent).toBe('IMPRESSIONS')
      
      const attributionSpec = JSON.parse(REFERENCE_ADSET_SETTINGS.attributionSpec)
