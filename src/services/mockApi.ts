...
class MockApiService {
  ...
  async getLocations(
    ...
    console.log('🔍 DEBUG: mockApi.getLocations called');
    console.log('🔍 DEBUG: mockLocations count:', mockLocations.length);
    console.log('🔍 DEBUG: mockLocations first few:', mockLocations.slice(0, 3).map(l => ({ id: l.id, name: l.name })));
    
    // Temporarily disable failure simulation for debugging
    // if (simulateFailure()) {
    ...
    // Apply filters
    if (filters?.search) {
      filteredLocations = searchMockLocations(filters.search);
    }
  ...
  async getLocationCities(state?: string): Promise<ApiResponse<string[]>> {
    ...
    let cities = getUniqueCities();
    
    if (state) {
      const locationsInState = getLocationsByState(state);
      cities = [...new Set(locationsInState.map(loc => loc.city))].sort();
    }
  ...
  async downloadGeneratedFile(jobId: string): Promise<ApiResponse<Blob>> {
    ...
    const csvContent = this.generateMockCsvContent(job);
  ...
  private substituteVariables(template: string, location: Location): string {
    return template
  ...
  private generateMockCsvContent(job: GenerationJob): string {
    ...
    for (const location of locations) {
      for (const template of templates) {
        const headline = this.substituteVariables(template.fields.headline, location);
        const description = this.substituteVariables(template.fields.description, location);
        const landingPageUrl = this.substituteVariables(template.fields.landingPageUrl, location);
        
        // Generate dynamic campaign name following the pattern
        ...
        const adSetName = `${campaignName}_${campaign.month}`;
        const adName = `${campaign.prefix}Spring25_${campaign.objective}_${campaign.testType}_${locationName}_6.26.25_BehindTheScenes`;
